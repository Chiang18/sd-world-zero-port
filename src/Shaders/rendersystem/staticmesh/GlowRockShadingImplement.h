//*************************************************************************************************
// 内容:	StaticMesh的浮空山石特殊处理,带GlowMap动画
//---------------------------------------------------------
// 作者:		
// 创建:		2012-07-19
// 最后修改:
...
struct VS_OUTPUT
{
	...
	float2	vUVSetGlowMap	: TEXCOORD5;
};
...
VS_OUTPUT VS_SHADER(VS_INPUT kInput)
{
	...
	kOutput.vUVSetGlowMap = mul(float4(kInput.vUVSetBase, 0.f, 1.f), g_mGlowMapTextureTransform).xy;
...
float4 PS_SHADER(VS_OUTPUT kInput) : COLOR0
{
	...
	float3 vGlowColor = tex2D(sdGlowSampler, kInput.vUVSetGlowMap).rgb;
	...
	// 合成光照
	float3 vColor = vDiffuseColor * (vDiffuseLight + g_vEmissiveMaterialColor) + vSpecularColor * vSpecularLight + vGlowColor;
...
// 着色技术
//---------------------------------------------------------------------------------------
#define TECHNIQUE HEADER_KEY(GlowRockShading, CHANNELS_FLAG)
technique TECHNIQUE
<
