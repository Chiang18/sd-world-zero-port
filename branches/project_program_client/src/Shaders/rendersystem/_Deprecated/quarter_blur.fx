...
{
	// 计算4领域纹理坐标
	float4 vUVSet0 = kInput.vUVSetBlur.xxyy + float4(-1, 1, -1, 1) * g_vHalfPixelOffset.xxyy;
	
	// 采样4领域纹理并混合
	float3 vBlurColor = 0;
	vBlurColor += 0.25f * tex2D(sdColorSampler, vUVSet0.xz).xyz;
	vBlurColor += 0.25f * tex2D(sdColorSampler, vUVSet0.yz).xyz;
	vBlurColor += 0.25f * tex2D(sdColorSampler, vUVSet0.xw).xyz;
	vBlurColor += 0.25f * tex2D(sdColorSampler, vUVSet0.yw).xyz;
...
// 着色技术
//---------------------------------------------------------------------------------------
technique PostEffect_QuarterBlur
<
	string Description = "PostEffect_QuarterBlur";
