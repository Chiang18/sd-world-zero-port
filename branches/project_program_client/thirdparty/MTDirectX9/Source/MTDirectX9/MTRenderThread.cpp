...
StateManager	DeviceState;						///< 渲染状态管理器
DWORD			g_uiMainThreadId	= 0;			///< 主线程
DWORD			g_uiRenderThreadId	= 0xffffffff;	///< 渲染线程
//-------------------------------------------------------------------------------------------------
// 渲染命令调试字符串
//-------------------------------------------------------------------------------------------------
char* strDebugAPI[] =
...
//	"enCF_Surface_UnlockRect",
	"enCF_Surface_Release",

	"enCF_CubeSurface_LockRect",
	"enCF_CubeSurface_UnlockRect",
	"enCF_CubeSurface_Release",
	...
	"enCF_VertexBuffer_Release",

	"enCF_IndexBuffer_Lock",
	"enCF_IndexBuffer_Unlock",
	"enCF_IndexBuffer_Release",

	"enCF_VertexDeclaration_Release",
...
//-------------------------------------------------------------------------------------------------
//IDirect3DDevice9
T_Device_Release					ST_Device_Release								= NULL;
...
//-------------------------------------------------------------------------------------------------
bool HasHooked(void* p)
{
	static std::set<void*> setHookApi;
	std::set<void*>::iterator itr = setHookApi.find(p);
	if (itr != setHookApi.end())
	{
		return	true;
	}
	else
...
//-------------------------------------------------------------------------------------------------
// Hook IDirect3DSurface9和IDirect3DTexture9
//-------------------------------------------------------------------------------------------------
void Hook_IDirect3DSurface9(IDirect3DDevice9* pDevice)
{
	...
	pDevice->lpVtbl->CreateTexture(pDevice, 1, 1, 1, D3DUSAGE_DYNAMIC, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &pTex, NULL);
	...
	ST_Surface_Release		= pSurface->lpVtbl->Release;

	ST_Texture_LockRect		= pTex->lpVtbl->LockRect;
	ST_Texture_UnlockRect	= pTex->lpVtbl->UnlockRect;
...
//		DetourAttach(&(PVOID&)		ST_Surface_UnlockRect	, MT_Surface_UnlockRect);
		DetourAttach(&(PVOID&)		ST_Surface_Release		, MT_Surface_Release);
		DetourAttach(&(PVOID&)		ST_Texture_LockRect		, MT_Texture_LockRect);
		DetourAttach(&(PVOID&)		ST_Texture_UnlockRect	, MT_Texture_UnlockRect);
	DetourTransactionCommit();

	ST_Surface_Release(pSurface);
	pTex->lpVtbl->Release(pTex);

	bHook_IDirect3DSurface9_Finished = true;
}
//-------------------------------------------------------------------------------------------------
// Hook IDirect3DSurface9
//-------------------------------------------------------------------------------------------------
void Hook_IDirect3DCubeSurface9(IDirect3DDevice9* pDevice)
{
	...
	pDevice->lpVtbl->CreateCubeTexture(pDevice, 1, 1, D3DUSAGE_DYNAMIC, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &pTex, NULL);
	...
	ST_CubeSurface_Release		= pSurface->lpVtbl->Release;
	
	DetourTransactionBegin();
	DetourUpdateThread(GetCurrentThread());
		DetourAttach(&(PVOID&)	ST_CubeSurface_LockRect				, MT_CubeSurface_LockRect);
		DetourAttach(&(PVOID&)	ST_CubeSurface_UnlockRect			, MT_CubeSurface_UnlockRect);
//		DetourAttach(&(PVOID&)	ST_CubeSurface_Release				, MT_CubeSurface_Release);
	DetourTransactionCommit();
...
//	pDevice->lpVtbl->CreateVolumeTexture(pDevice, 1,1,1,1,D3DUSAGE_DYNAMIC,D3DFMT_A8R8G8B8,D3DPOOL_DEFAULT,&pTex,NULL);
...
//	ST_Volume_Release		= pVolume->lpVtbl->Release;
//
//	DetourTransactionBegin();
//	DetourUpdateThread(GetCurrentThread());
//		DetourAttach(&(PVOID&)	ST_Volume_LockBox			,MT_Volume_LockBox);
//		DetourAttach(&(PVOID&)	ST_Volume_UnlockBox			,MT_Volume_UnlockBox);
////		DetourAttach(&(PVOID&)	ST_VolumeRelease			,MT_Volume_Release);
//	DetourTransactionCommit();
//
//	ST_Surface_Release((IDirect3DSurface9*)pVolume);
//	pTex->lpVtbl->Release(pTex);
//
//	bHook_IDirect3DVolume9_Finished	=	true;
//};
//-------------------------------------------------------------------------------------------------
// Hook IDirect3DVertexBuffer9
//-------------------------------------------------------------------------------------------------
void Hook_IDirect3DVertexBuffer9(IDirect3DDevice9* pDevice)
{
	...
	pDevice->lpVtbl->CreateVertexBuffer(pDevice, 32, D3DUSAGE_DYNAMIC, 0, D3DPOOL_DEFAULT, &pVB, 0);
	ST_VertexBuffer_Lock		= pVB->lpVtbl->Lock;
	ST_VertexBuffer_Unlock		= pVB->lpVtbl->Unlock;
	ST_VertexBuffer_Release		= pVB->lpVtbl->Release;

	DetourTransactionBegin();
	DetourUpdateThread(GetCurrentThread());
		if(!HasHooked(ST_VertexBuffer_Lock))
			DetourAttach(&(PVOID&)	ST_VertexBuffer_Lock			, MT_VertexBuffer_Lock);
		if(!HasHooked(ST_VertexBuffer_Unlock))
			DetourAttach(&(PVOID&)	ST_VertexBuffer_Unlock			, MT_VertexBuffer_Unlock);
		DetourAttach(&(PVOID&)	ST_VertexBuffer_Release				, MT_VertexBuffer_Release);
	DetourTransactionCommit();

	ST_VertexBuffer_Release(pVB);

	bHook_IDirect3DVertexBuffer9_Finished =	true;
}
//-------------------------------------------------------------------------------------------------
// Hook IDirect3DIndexBuffer9
//-------------------------------------------------------------------------------------------------
void Hook_IDirect3DIndexBuffer9(IDirect3DDevice9* pDevice)
{
	...
	pDevice->lpVtbl->CreateIndexBuffer(pDevice, 32, D3DUSAGE_DYNAMIC, D3DFMT_INDEX16, D3DPOOL_DEFAULT, &pIB, 0);
	ST_IndexBuffer_Lock			= pIB->lpVtbl->Lock;
	ST_IndexBuffer_Unlock		= pIB->lpVtbl->Unlock;
	ST_IndexBuffer_Release		= pIB->lpVtbl->Release;

	DetourTransactionBegin();
	DetourUpdateThread(GetCurrentThread());
		if(!HasHooked(ST_IndexBuffer_Lock))
			DetourAttach(&(PVOID&)	ST_IndexBuffer_Lock		, MT_IndexBuffer_Lock);
		if(!HasHooked(ST_IndexBuffer_Unlock))
			DetourAttach(&(PVOID&)	ST_IndexBuffer_Unlock	, MT_IndexBuffer_Unlock);
//		DetourAttach(&(PVOID&)	ST_IndexBuffer_Release	, MT_IndexBuffer_Release);
	DetourTransactionCommit();

	ST_VertexBuffer_Release((IDirect3DVertexBuffer9*) pIB);

	bHook_IDirect3DIndexBuffer9_Finished = true;
}
//-------------------------------------------------------------------------------------------------
// Hook IDirect3DStateBlock9
//-------------------------------------------------------------------------------------------------
void Hook_IDirect3DStateBlock9(IDirect3DDevice9* pDevice)
{
	if (bHook_IDirect3DStateBlock9_Finished)
		return;
	...
	bHook_IDirect3DStateBlock9_Finished	= true;
}
//-------------------------------------------------------------------------------------------------
// Hook IDirect3DSwapChain9
//-------------------------------------------------------------------------------------------------
...
{	
	...
	// Hook
	ST_Device_Release						= pThis->lpVtbl->Release;
	...
	DetourTransactionBegin();
	DetourUpdateThread(GetCurrentThread());
		DetourAttach(&(PVOID&)	ST_Device_Release					 ,MT_Device_Release);
...
//		DetourAttach(&(PVOID&)	ST_CreateQuery						 ,MT_CreateQuery);
	...
	Hook_IDirect3DSurface9(pThis);			///< HOOK SURFACE
	Hook_IDirect3DCubeSurface9(pThis);		///< HOOK CUBESURFACE
	//Hook_IDirect3DVolume9(pThis);			///< HOOK VOLUME
	Hook_IDirect3DVertexBuffer9(pThis);		///< HOOK VERTEXBUFFER
	Hook_IDirect3DIndexBuffer9(pThis);		///< HOOK INDEXBUFFER
	//Hook_IDirect3DStateBlock9(pThis);		///< HOOK STATEBLOCK
	//Hook_IDirect3DSwapChain9(pThis);		///< HOOK SWAPCHAIN
...
void UnHook_IDirect3DDevice9()
{
	...
	DetourTransactionBegin();
	DetourUpdateThread(GetCurrentThread());
		DetourDetach(&(PVOID&)	ST_Device_Release					 ,MT_Device_Release);
...
MT_RenderThread::~MT_RenderThread()
{
	...
	// 释放所有顶点声明
	MTD3DVD9VecItr itr = m_kMTD3DVD9Vec.begin();
	MTD3DVD9VecItr itr_end = m_kMTD3DVD9Vec.end();
	for (; itr != itr_end; ++itr)
	{
		MT_IDirect3DVertexDeclaration9*	p =	*itr;
		T_Release(p->m_pIDirect3DVertexDeclaration9);
		delete p;
	}
	m_kMTD3DVD9Vec.clear();

	StopThreadWaitTimeOut(2000);
};
//-------------------------------------------------------------------------------------------------
HRESULT	MT_RenderThread::Swap()
{
	// 等待渲染完毕
	m_evtMainWaitRender.Wait();

	// 检查设备是否丢失
	HRESULT hr = ST_TestCooperativeLevel(m_pDevice);
	m_bDeviceLost = (hr != S_OK);
...
//-------------------------------------------------------------------------------------------------
#define	READ_COMMAND_STRUCT(CmdStruct)			\
	CmdStruct* param = (CmdStruct*)pBuffer;		\
	pBuffer += sizeof(CmdStruct);
//-------------------------------------------------------------------------------------------------
void MT_RenderThread::RenderFrame()
{
	HRESULT hr = S_OK;

	// Before Render, We Check the Device Lost, If Device isn't OK! return it.
	//hr = ST_TestCooperativeLevel(m_pDevice);
	//if (hr != D3D_OK)
	//	return;
	...
	U8*	pBuffer	= (U8*)m_DBuffer.Read(4);	///< 当前CommandBuffer
	//U8* pStart	= pBuffer;					
	//static int iFrame	=	0;
	//char strTemp[128];
	//sprintf_s(strTemp,128,"Frame[%d]\n",++iFrame);
	//OutputDebugStringA(strTemp);
	int iBeginStateBlock =	0;
	static bool	bDebugFunc = false;
	while(true)
	{
		hr = S_OK;
		...
		{
			::OutputDebugStringA("\n");
			::OutputDebugStringA(strDebugAPI[func]);
		}
		...
		switch(func)
		{
			...
			case enCF_Present:
				{
					...
					if (pSourceRect!=NULL)	pSourceRect	= &param->src;
					if (pDestRect!=NULL)	pDestRect =	&param->dst;
					if (pData!=NULL)		pData =	&param->dirty;
			...
			case enCF_UpdateSurface:
				{
					...
					if (pSourceRect!=NULL)	pSourceRect	= &param->src;
					if (pDestPoint!=NULL)	pDestPoint	= &param->dst;
			...
			case enCF_GetRenderTargetData:
				{
					READ_COMMAND_STRUCT(CmdGetRenderTargetData);
					hr = ST_GetRenderTargetData(m_pDevice, param->pRenderTarget, param->pDestSurface);	
				}break;
			...
			case enCF_SetRenderTarget:
				{
					...
					//if (FAILED(hr))
					//{
					//	//OutputDebugStringA("SetRenderTarget Problem!\n");
					//	D3DSURFACE_DESC	desc;
					//	param->pRenderTarget->lpVtbl->GetDesc(param->pRenderTarget,&desc);
					//	//sprintf_s(strTemp,128,"DEBUG_INFO <SetRenderTarget FAILED!>[ w=%d h=%d fmt=%d ]\n",desc.Width,desc.Height,desc.Format);
					//	//::OutputDebugStringA(strTemp);
					//}
				}break;
			...
			case enCF_EndStateBlock:
				{
					...
					MT_IDirect3DStateBlock9*	p	=	static_cast<MT_IDirect3DStateBlock9*>(*param);
					iBeginStateBlock--;
					if (iBeginStateBlock==0)
					{
						hr = ST_EndStateBlock(m_pDevice,&p->m_pIDirect3DStateBlock9);
						//p->m_pIDirect3DStateBlock9->lpVtbl->
					}
					else
					{
						iBeginStateBlock	=	0;
			...
			case enCF_DrawPrimitiveUP:{
				READ_COMMAND_STRUCT(CmdDrawPrimitiveUP);
				hr	=	ST_DrawPrimitiveUP(m_pDevice,param->PrimitiveType,param->PrimitiveCount,pBuffer,param->VertexStreamZeroStride);
				pBuffer+=param->uiBufferSize;
									  }break;
			case enCF_DrawIndexedPrimitiveUP:{
				READ_COMMAND_STRUCT(CmdDrawIndexedPrimitiveUP);
				
				
				void*	pVB	=	pBuffer;
				pBuffer+=param->uiVBufferSize; 
				void*	pIB	=	pBuffer;
				pBuffer+=param->uiIBufferSize; 

				hr	=	ST_DrawIndexedPrimitiveUP(	m_pDevice,
													param->PrimitiveType,
			...
			case enCF_SetVertexDeclaration:
				{
					...
					{
						// 设置VD
						MT_IDirect3DVertexDeclaration9*	p = static_cast<MT_IDirect3DVertexDeclaration9*>(*param);
						hr = ST_SetVertexDeclaration(m_pDevice, p->GetVD());
					}
				}break;
			...
			case enCF_Surface_LockRect:
				{
					...
					{
						pRect = &param->rData;
						uiHeight = param->rData.bottom - param->rData.top;
					}
					else
					{
						D3DSURFACE_DESC	desc;
						param->pSurface->lpVtbl->GetDesc(param->pSurface, &desc);
						uiHeight = desc.Height;
					}
					...
					if(SUCCEEDED(hr))
					{
						...
						U8*	pSrc = pBuffer;
						U8*	pDst = (U8*)rLock.pBits;
						U32	uiCopySize = min(param->pitch, (U32)rLock.Pitch);
						for(U32	i=0;i<uiHeight;i++)
						{
							memcpy(pDst, pSrc, uiCopySize);
							pSrc += param->pitch;
							pDst += rLock.Pitch;
						}
					}
					else
					{
						...
						case D3DERR_INVALIDCALL:
							{
								::OutputDebugStringA("Surface_LockRect D3DERR_INVALIDCALL\n");
							}break;
						case D3DERR_WASSTILLDRAWING:
							{
								::OutputDebugStringA("Surface_LockRect D3DERR_WASSTILLDRAWING\n");
							}break;
						}
					}
			...
			case enCF_CubeSurface_LockRect:
				{
					READ_COMMAND_STRUCT(CmdSurface_LockRect);
					RECT* pRect		= (RECT*)param->pRect;
					U32 uiHeight	= 0;
					...
					D3DLOCKED_RECT rLock;
					hr = ST_CubeSurface_LockRect(param->pSurface, &rLock, pRect, param->Flags);
					if(SUCCEEDED(hr))
					{
						U8*	pSrc = pBuffer;
						U8*	pDst = (U8*)rLock.pBits;
			...
			case enCF_CubeSurface_UnlockRect:
				{
					READ_COMMAND_STRUCT(CmdSurface_UnlockRect);
					hr = ST_CubeSurface_UnlockRect(*param);
				}break;
			...
			case enCF_VertexBuffer_Lock:{
				...
				if(SUCCEEDED(hr)){
					if(pData==NULL){
						OutputDebugStringA("VertexBuffer Lock Return NULL Buffer!\n");
					}else{
						memcpy(pData,pBuffer,param->uiBufferSize);
					}
				}
				pBuffer+=param->uiBufferSize;
										}break;
			...
			case enCF_IndexBuffer_Lock:
				{
					...
					{
						if (pData == NULL)
							OutputDebugStringA("IndexBuffer Lock Return NULL Buffer!\n");
						else
							memcpy(pData,pBuffer,param->uiBufferSize);
					}
					pBuffer += param->uiBufferSize;
			...
			case enCF_Texture_LockRect:
				{
					...
					D3DSURFACE_DESC desc;
					param->pTex->lpVtbl->GetLevelDesc(param->pTex, param->Level, &desc);
					...
					D3DLOCKED_RECT rLock;
					hr = ST_Texture_LockRect(param->pTex, param->Level, &rLock, pRect, param->Flags);
					if (SUCCEEDED(hr))
					{
						U8* pSrc = pBuffer;
						U8* pDst = (U8*)rLock.pBits;
						...
						case D3DFMT_DXT5:
							{
								memcpy(pDst, pSrc, param->dwLockSize);	///< 暂时直接拷贝
							}break;

						default:
							{
								if (rLock.Pitch == param->pitch)
								{
									memcpy(pDst, pSrc, param->dwLockSize);	///< Pitch相同,直接拷贝
								}
								else
								{
									U32 uiCopySize = min(param->pitch, (U32)rLock.Pitch);
			...
			case enCF_StateBlock_Apply:
				{
					...
					MT_IDirect3DStateBlock9* pMTState =	static_cast<MT_IDirect3DStateBlock9*>(*param);

					// 前面的BeginEnd 调用顺序问题导致IDirect3DStateBlock9 创建失败Capture就会无效
					if (pMTState->m_pIDirect3DStateBlock9 != NULL)
						hr = pMTState->m_pIDirect3DStateBlock9->lpVtbl->Apply(pMTState->m_pIDirect3DStateBlock9);	//->Apply();					
	
					//sprintf_s(strTemp,128,"(%x)",*param);
					//OutputDebugStringA(strTemp);
					//T_Release(*param);
				}break;
			...
			case enCF_StateBlock_Capture:
				{
					...
					//前面调用的EndStateBlock可能失败,如果失败,建立StateBlock 
					if (pMTState->m_pIDirect3DStateBlock9 == NULL)
						hr = ST_CreateStateBlock(m_pDevice,pMTState->m_Type,&pMTState->m_pIDirect3DStateBlock9);
					if (pMTState->m_pIDirect3DStateBlock9 != NULL)
						hr = pMTState->m_pIDirect3DStateBlock9->lpVtbl->Capture(pMTState->m_pIDirect3DStateBlock9);	//->Capture();	
...
//			case enCF_Effect_SetValue:
//				{
//					//(THIS_ D3DXHANDLE hParameter, LPCVOID pData, UINT Bytes);
...
//				}break;
//
//			case enCF_Effect_SetBool:
//				{
//					//(THIS_ D3DXHANDLE hParameter, BOOL b);
//					READ_COMMAND_STRUCT(CmdEffect_SetBool);
//					hr = param->pID3DXEffect->lpVtbl->SetBool(param->pID3DXEffect, param->hParameter, param->b);
//				}break;
//
//			case enCF_Effect_SetBoolArray:
//				{
//					//(THIS_ D3DXHANDLE hParameter, CONST BOOL* pb, UINT Count);
...
//				}break;
//
//			case enCF_Effect_SetInt:
//				{
//					//(THIS_ D3DXHANDLE hParameter, INT n);
//					READ_COMMAND_STRUCT(CmdEffect_SetInt);
//					hr = param->pID3DXEffect->lpVtbl->SetInt(param->pID3DXEffect, param->hParameter, param->n);
//				}break;
//
//			case enCF_Effect_SetIntArray:
//				{
//					//(THIS_ D3DXHANDLE hParameter, CONST INT* pn, UINT Count);
...
//				}break;
//
//			case enCF_Effect_SetFloat:
//				{
//					//(THIS_ D3DXHANDLE hParameter, FLOAT f);
//					READ_COMMAND_STRUCT(CmdEffect_SetFloat);
//					hr = param->pID3DXEffect->lpVtbl->SetFloat(param->pID3DXEffect, param->hParameter, param->f);
//				}break;
//
//			case enCF_Effect_SetFloatArray:
//				{
//					//(THIS_ D3DXHANDLE hParameter, CONST FLOAT* pf, UINT Count);
...
//				}break;
//
//			case enCF_Effect_SetVector:
//				{
//					//(THIS_ D3DXHANDLE hParameter, CONST D3DXVECTOR4* pVector);
...
//				}break;
//
//			case enCF_Effect_SetVectorArray:
//				{
//					//(THIS_ D3DXHANDLE hParameter, CONST D3DXVECTOR4* pVector, UINT Count);
...
//				}break;
//
//			case enCF_Effect_SetMatrix:
//				{
//					//(THIS_ D3DXHANDLE hParameter, CONST D3DXMATRIX* pMatrix);
...
//				}break;
//
//			case enCF_Effect_SetMatrixArray:
//				{
...
//				}break;
//
//			case enCF_Effect_SetMatrixPointerArray:
//				{
//					//(THIS_ D3DXHANDLE hParameter, CONST D3DXMATRIX** ppMatrix, UINT Count);
//				}break;
//
//			case enCF_Effect_SetMatrixTranspose:
//				{
//					//(THIS_ D3DXHANDLE hParameter, CONST D3DXMATRIX* pMatrix);
...
//				}break;
//
//			case enCF_Effect_SetMatrixTransposeArray:
//				{
//					//(THIS_ D3DXHANDLE hParameter, CONST D3DXMATRIX* pMatrix, UINT Count);
...
//				}break;
//
//			case enCF_Effect_SetMatrixTransposePointerArray:
//				{
//					//(THIS_ D3DXHANDLE hParameter, CONST D3DXMATRIX** ppMatrix, UINT Count);
//				}break;
//
//			case enCF_Effect_SetString:
//				{
//					//(THIS_ D3DXHANDLE hParameter, LPCSTR pString);
...
//				}break;
//
//			case enCF_Effect_SetTexture:
//				{
//					//(THIS_ D3DXHANDLE hParameter, LPDIRECT3DBASETEXTURE9 pTexture);
...
//				}break;
//
//			case enCF_Effect_CommitChanges:
//				{
//					//(THIS);
...
//				}break;
//
//			case enCF_Effect_SetStateManager:
//				{
...
//				}break;
//
//			case enCF_Effect_SetRawValue:
//				{
//					//(THIS_ D3DXHANDLE hParameter, LPCVOID pData, UINT ByteOffset, UINT Bytes);
...
//					hr = ST_D3DPERF_BeginEvent(param->col, pStr);
//				}break;
//
//			case enCF_PIX_D3DPERF_EndEvent:
// 				{
//					hr = ST_D3DPERF_EndEvent();
//				}break;
//
...
//					wchar_t* pStr = (wchar_t*)pBuffer;
//					pBuffer += param->dwNameSize;
// 					ST_D3DPERF_SetMarker(param->col, pStr);
//				}break;

			case enCF_CommandEnd:
				{
					// 指令结束,跳出
					return;
				}break;
		...
		{
			::OutputDebugStringA(strDebugAPI[func]);
			::OutputDebugStringA(" Call Failed!\n");
			//DebugFrame(pStart,pBuffer);
			//bDebugFunc = true;
		}
	}
};
...
{
	g_uiRenderThreadId	= m_iThreadID;

	// 如果主线程调用StopThread, 则需要不再执行内部逻辑, 也不进入等待
	if(!m_bExit)
	{
		//
		m_State	= enRTS_Waiting;
...
//-------------------------------------------------------------------------------------------------
void MT_RenderThread::Sync()
{
	m_DBuffer.Request(enCF_CommandEnd);
	...
	//WaitRenderComplated();
	while (m_State!=enRTS_Waiting)
		OutputDebugStringA("Wating...\n");
}
//-------------------------------------------------------------------------------------------------
void MT_RenderThread::DebugRenderThread()
{
	m_DBuffer.Sync();	///< 交换
	
	DWORD uiOldThreadId	= g_uiMainThreadId;
	g_uiMainThreadId = g_uiRenderThreadId;

	RenderFrame();
	ReleaseRenderObject();

	g_uiMainThreadId=uiOldThreadId;
}
//-------------------------------------------------------------------------------------------------
void MT_RenderThread::ReleaseRenderObject()
{
	ReleaseObjVector& kReadReleaseObjVec = m_DBuffer.GetReadReleaseObjVector();
	ReleaseObjVectorItr itr = kReadReleaseObjVec.begin();
	ReleaseObjVectorItr itr_end = kReadReleaseObjVec.end();
	for(; itr != itr_end; ++itr)
		ReleaseObject(*itr);
	kReadReleaseObjVec.clear();
}
//-------------------------------------------------------------------------------------------------
void MT_RenderThread::ReleaseWriteObject()
{
	ReleaseObjVector& kWriteReleaseObjVec = m_DBuffer.GetWriteReleaseObjVector();
	ReleaseObjVectorItr itr = kWriteReleaseObjVec.begin();
	ReleaseObjVectorItr itr_end = kWriteReleaseObjVec.end();
	for(; itr != itr_end; ++itr)
		ReleaseObject(*itr);
	kWriteReleaseObjVec.clear();
}
//-------------------------------------------------------------------------------------------------
void MT_RenderThread::ReleaseAllObject()
{
	ReleaseObjVector& kReadReleaseObjVec = m_DBuffer.GetReadReleaseObjVector();
	ReleaseObjVectorItr itr = kReadReleaseObjVec.begin();
	ReleaseObjVectorItr itr_end = kReadReleaseObjVec.end();
	...
	ReleaseObjVector& kWriteReleaseObjVec = m_DBuffer.GetWriteReleaseObjVector();
	itr = kWriteReleaseObjVec.begin();
	itr_end = kWriteReleaseObjVec.end();
	for(; itr != itr_end; ++itr)
		ReleaseObject(*itr);
	kWriteReleaseObjVec.clear();
}
//-------------------------------------------------------------------------------------------------
void MT_RenderThread::ReleaseObject(ReleaseObj& obj)
{
	switch(obj.func)
	{
		case enCF_Surface_Release:
			{
				ST_Surface_Release((IDirect3DSurface9*)obj.pObj);
			}break;

		case enCF_CubeSurface_Release:
			{
				ST_CubeSurface_Release((IDirect3DSurface9*)obj.pObj);					 
			}break;

		case enCF_Volume_Release:
			{
				ST_Volume_Release((IDirect3DVolume9*)obj.pObj);	
			}break;

		case enCF_VertexBuffer_Release:
			{
				ST_VertexBuffer_Release((IDirect3DVertexBuffer9*)obj.pObj);	
			}break;

		case enCF_IndexBuffer_Release:
			{
				ST_IndexBuffer_Release((IDirect3DIndexBuffer9*)obj.pObj);	
			}break;

		case enCF_VertexDeclaration_Release:
			{
				ST_VertexDeclaration_Release((IDirect3DVertexDeclaration9*)obj.pObj);	
			}break;

		case enCF_StateBlock_Release:
			{
				MT_IDirect3DStateBlock9* pStateBlock = (MT_IDirect3DStateBlock9*)obj.pObj;
				T_Release(pStateBlock->m_pIDirect3DStateBlock9);
				//ST_StateBlock_Release(pStateBlock->m_pIDirect3DStateBlock9);
				delete pStateBlock;
			}break;
	}
}
//-------------------------------------------------------------------------------------------------
void MT_RenderThread::DebugFrame(U8* pBuffer, U8* pEnd)
{
/*	HRESULT	 hr	= S_OK;
			...
			case enCF_StateBlock_Apply:
				{
					...
					//	hr	=	pMTState->m_pIDirect3DStateBlock9->lpVtbl->Apply(pMTState->m_pIDirect3DStateBlock9);//->Apply();
						...
						//前面的BeginEnd 调用顺序问题导致IDirect3DStateBlock9 创建失败 Capture 就会无效
					}
					//sprintf_s(strTemp,128,"(%x)",*param);
					//OutputDebugStringA(strTemp);
					//T_Release(*param);
...
//-------------------------------------------------------------------------------------------------
MT_IDirect3DVertexDeclaration9*	MT_RenderThread::CreateVD(CONST D3DVERTEXELEMENT9*	pVertexElements)
{
	// 获取数据流声明数量
	// (D3D9的最大长度是MAXD3DDECLLENGTH = 64(d3d9types.h),这里直接用16可能会有问题,合理的值是64+1)
	U32	uiNumElement = 0;
	for (int i = 0; i < 16; i++)
	{
	...
	for (; itr != itr_end; ++itr)
	{
		...
		if (p != NULL)
		{
			U32	uiTempElementCount = p->m_kElementVec.size();
			if (uiTempElementCount != uiNumElement)
				continue;
			if (memcmp(pVertexElements, &p->m_kElementVec[0], sizeof(D3DVERTEXELEMENT9) * uiNumElement) == 0)
				return	p;			
		}
	}

	// 创建顶点声明,并加入缓存
	MT_IDirect3DVertexDeclaration9*	p =	NewMT_IDirect3DVertexDeclaration9(m_pDevice, pVertexElements, uiNumElement);
	m_kMTD3DVD9Vec.push_back(p);
