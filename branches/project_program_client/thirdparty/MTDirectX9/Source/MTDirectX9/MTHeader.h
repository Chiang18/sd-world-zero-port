...
// 参考点objbase.h和d3d9.h头文件
//	c++ interface 使用类
//	c interface 使用函数表和宏定义从形式上模拟类
#define CINTERFACE
...
{
public:
	Event(const std::string& strName = "")
	{
...
struct MT_IDirect3DStateBlock9;
struct MT_IDirect3DVertexDeclaration9;
...
// 函数指针原型
//-----------------------------------------------------------------------------
typedef	ULONG  (STDMETHODCALLTYPE *T_Device_Release)(IDirect3DDevice9* pThis);
...
typedef	ULONG	(STDMETHODCALLTYPE *T_Surface_Release)(IDirect3DSurface9* pThis);
typedef	ULONG	(STDMETHODCALLTYPE *T_CubeSurface_Release)(IDirect3DSurface9* pThis);
...
typedef	ULONG	(STDMETHODCALLTYPE *T_Volume_Release)(IDirect3DVolume9* pThis);
...
typedef	ULONG	(STDMETHODCALLTYPE *T_VertexBuffer_Release)(IDirect3DVertexBuffer9* pThis);
...
typedef	ULONG	(STDMETHODCALLTYPE *T_IndexBuffer_Release)(IDirect3DIndexBuffer9* pThis);

typedef	ULONG	(STDMETHODCALLTYPE *T_VertexDeclaration_Release)(IDirect3DVertexDeclaration9* pThis);
...
typedef	ULONG	(STDMETHODCALLTYPE *T_StateBlock_Release)(IDirect3DStateBlock9* pThis);
...
//------------------------------------------------------------------------------
typedef	IDirect3DDevice9*				CmdDeviceRelease;
typedef IDirect3DSurface9*				CmdSurfaceRelease;
typedef IDirect3DSurface9*				CmdCubeSurfaceRelease;
typedef IDirect3DVolume9*				CmdVolumeRelease;
typedef IDirect3DVertexBuffer9*			CmdVertexBufferRelease;
typedef IDirect3DIndexBuffer9*			CmdIndexBufferRelease;
typedef IDirect3DVertexDeclaration9*	CmdVertexDeclarationRelease;
typedef IDirect3DStateBlock9*			CmdStateBlockRelease;
...
struct ReleaseObj
{
	enumCommandFunc	func;
	void*			pFunc;
	void*			pObj;
};
typedef	std::vector<ReleaseObj>	ReleaseObjVector;
typedef	std::vector<ReleaseObj>::iterator ReleaseObjVectorItr;
//------------------------------------------------------------------------------
#endif