...
class NxFluidDescBase
	...
	*/
	...
	Must be 0 or greater.
 	...
 	The value defines a ratio between tangential and normal acceleration, at which particles start to slide on a surface. 
 	If "a[T] > a[N]*staticCollisionStiction" holds, the particle can slide, a[T] beeing the tangential acceleration and a[N]
 	beeing the normal acceleration.
	*/
...
NX_INLINE bool NxFluidDescBase::isValid() const
	...
	if (staticFrictionForDynamicShapes < 0.0f) return false;
	...
	if (staticFrictionForStaticShapes < 0.0f) return false;
