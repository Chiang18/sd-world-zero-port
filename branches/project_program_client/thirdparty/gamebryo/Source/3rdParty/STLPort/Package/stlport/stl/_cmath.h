...
#else
#  if !defined (_STLP_CMATH_FUNC_NAMESPACE)
 ...
 * in global namespace.
 */
 // TODO: The EE_STLP_MATH_ALREADY_GLOBAL check is not desirable.  It would be better to use an STLPORT 
 // defined flag.  Currently the only other way to avoid the inlines below requires us to define _STLP_MSVC_LIB which 
 // does not seem right for a PS3.  The reason we want to avoid the inlines below it that they cause duplicate definitions
 // for the PS3 when combined with the PS3 version of math.h.  There have been several other attempts to fix this
 // issue in the past (including defining _STLP_MSVC_LIB 1400) and there may well be a better way, but, I could 
 // not find it.
#if (!defined (_STLP_MSVC_LIB) || (_STLP_MSVC_LIB < 1310) || defined(UNDER_CE)) && !defined(EE_STLP_MATH_ALREADY_GLOBAL)
