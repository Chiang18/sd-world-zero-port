...
void hashtable<_Val,_Key,_HF,_Traits,_ExK,_EqK,_All>
  ...
  if (__cur == __first._M_ite) {
    ...
    // Custom Modification: Renamed scope-local __last to avoid a shadow
    // parameter warning on GCC [mnoland 9/4/2008] 
    _ElemsIte __last2(_M_buckets[++__f_bucket]);
    __prev = __cur++;
    for (; (__cur != __last2) && (__cur != __first._M_ite); ++__prev, ++__cur);
