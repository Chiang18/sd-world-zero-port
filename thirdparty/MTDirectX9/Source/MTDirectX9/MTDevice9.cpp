...
//-------------------------------------------------------------------------------------------------
// D3DRENDERSTATETYPE的扩展
//-------------------------------------------------------------------------------------------------
enum D3DRENDERSTATETYPEEX
{
	D3DRS_EX_FORCE_FRONT_THREAD = 210,
	D3DRS_EX_FLUSH,
	D3DRS_EX_SYNC,
	D3DRS_EX_RES_MEMORY_INFO,
	D3DRS_EX_RES_HEAP_INFO,
	D3DRS_EX_RES_HEAP_BLOCK_INFO,
	
	D3DRS_EX_MAX,
};
DWORD g_D3DRSEX[D3DRS_EX_MAX] = { FALSE };
...
// D3D原函数指针(外部声明)
//-------------------------------------------------------------------------------------------------
extern T_Device_Release					ST_Device_Release;
...
{
	ULONG uiRef = ST_Device_Release(pThis);	// *(ULONG*)(((unsigned char*)pThis)+0x34);
	//if(uiRef==1)
	//{
...
HRESULT (STDMETHODCALLTYPE MT_TestCooperativeLevel)(IDirect3DDevice9* pThis)
{
	...
	if (MT_RenderThread::GetSingleton().IsDeviceLost())
	{
		...
		MT_RenderThread::GetSingleton().ReleaseWriteObject();
	}

	HRESULT	hr = ST_TestCooperativeLevel(pThis);

	// 设置设备状态
	MT_RenderThread::GetSingleton().SetDeviceLost(hr != S_OK);

	// 输出调试字符串
	switch(hr)
	{
	case D3DERR_DEVICELOST :
		::OutputDebugStringA("MT DEVICE LOST!\n");
		break;

	case D3DERR_DEVICENOTRESET :
		::OutputDebugStringA("MT DEVICE NOT RESET!\n");
		break;

	case D3DERR_DRIVERINTERNALERROR :
		::OutputDebugStringA("MT DEVICE DRIVER INTERNAL ERROR\n");
		break;
	}
...
{
	return ST_GetSwapChain(pThis, iSwapChain, pSwapChain);
};
//-------------------------------------------------------------------------------------------------
...
HRESULT	STDMETHODCALLTYPE MT_Reset(IDirect3DDevice9* pThis, D3DPRESENT_PARAMETERS* pPresentationParameters)
{
	...
	DWORD dwOldID = g_uiMainThreadId;
	g_uiMainThreadId = 0;
	HRESULT	hr = ST_Reset(pThis, pPresentationParameters);
	if(hr != S_OK)	///< Win7下需要多进行尝试RESET,否则数据可能错误,界面文字花掉
		hr = ST_Reset(pThis, pPresentationParameters);
	g_uiMainThreadId = dwOldID;
...
HRESULT	STDMETHODCALLTYPE MT_Present(IDirect3DDevice9* pThis, CONST RECT* pSourceRect, CONST RECT* pDestRect, HWND hDestWindowOverride, CONST RGNDATA* pDirtyRegion)
{
	...
	if (pSourceRect != NULL)	param->src = *pSourceRect;
	if (pDestRect != NULL)		param->dst = *pDestRect;
	if (pDirtyRegion != NULL)	param->dirty = *pDirtyRegion;
...
HRESULT STDMETHODCALLTYPE MT_UpdateSurface(IDirect3DDevice9* pThis, IDirect3DSurface9* pSourceSurface, CONST RECT* pSourceRect, IDirect3DSurface9* pDestinationSurface, CONST POINT* pDestPoint)
{
	...
	if (pSourceRect != NULL)	param->src = *pSourceRect;
	if (pDestPoint != NULL)		param->dst = *pDestPoint;
	return	S_OK;
};
//-------------------------------------------------------------------------------------------------
...
{
	if (g_D3DRSEX[D3DRS_EX_FORCE_FRONT_THREAD])
	{
		while (MT_RenderThread::GetSingleton().GetRenderThreadState() != enRTS_Waiting)
			return ST_GetRenderTargetData(pThis, pRenderTarget, pDestSurface);
	}
...
{
	if (g_D3DRSEX[D3DRS_EX_FORCE_FRONT_THREAD])
		return ST_StretchRect(pThis, pSourceSurface, pSourceRect, pDestSurface, pDestRect, Filter);
...
//-------------------------------------------------------------------------------------------------
//const GUID MT_IID_IDirect3DSurface9= 
//{ 
//	0xcfbaf3a, 
//	0x9ff6, 
//	0x429a, 
//	{ 0x99, 0xb3, 0xa2, 0x79, 0x6a, 0xf8, 0xb8, 0x9b } 
//};
//-------------------------------------------------------------------------------------------------
...
HRESULT STDMETHODCALLTYPE MT_GetDepthStencilSurface(IDirect3DDevice9* pThis, IDirect3DSurface9** ppZStencilSurface)
{
	...
	if (DeviceState.m_pDepthStencil == NULL)
		return 0x88760866;
...
HRESULT STDMETHODCALLTYPE MT_CreateVertexDeclaration(IDirect3DDevice9* pThis, CONST D3DVERTEXELEMENT9* pVertexElements, IDirect3DVertexDeclaration9** ppDecl)
{
	...
	// 由渲染线程创建顶点声明(渲染线程内部对顶点声明进行缓存和复用)
	*ppDecl	= MT_RenderThread::GetSingleton().CreateVD(pVertexElements);

	return S_OK;
};
//-------------------------------------------------------------------------------------------------
extern IDirect3DVertexDeclaration9Vtbl MT_lpVtbl;	///< IDirect3DVertexDeclaration9函数指针表
HRESULT STDMETHODCALLTYPE MT_SetVertexDeclaration(IDirect3DDevice9* pThis, IDirect3DVertexDeclaration9* pDecl)
{
	// 渲染线程情况: 直接处理
	if (g_uiMainThreadId != ::GetCurrentThreadId())
	{
		if (pDecl->lpVtbl == &MT_lpVtbl)
		{
			MT_IDirect3DVertexDeclaration9* p = static_cast<MT_IDirect3DVertexDeclaration9*>(pDecl);
			if (p == NULL)
				return ST_SetVertexDeclaration(pThis, pDecl);
			else
				return ST_SetVertexDeclaration(pThis, p->GetVD());
		}
		else
		{
			return ST_SetVertexDeclaration(pThis, pDecl);
	...
	*param = pDecl;
	...
	DeviceState.m_pVD =	pDecl;		///< 提前设置给DeviceStateBuff
...
{
	*ppDecl	= DeviceState.m_pVD;	///< 直接从DeviceStateBuff取
...
HRESULT STDMETHODCALLTYPE MT_SetFVF(IDirect3DDevice9* pThis, DWORD FVF)
{
	...
	*param = FVF;
	DeviceState.m_FVF =	FVF;	///< 提前设置给DeviceStateBuff(DWORD值没有引用计数)
...
{
	*pFVF =	DeviceState.m_FVF;	///< 直接从DeviceStateBuff取
	return S_OK;
};
//-------------------------------------------------------------------------------------------------
...
ULONG STDMETHODCALLTYPE	MT_Surface_Release(IDirect3DSurface9* pThis)
{
	...
	{
		if (g_uiMainThreadId != GetCurrentThreadId())
			return ST_Surface_Release(pThis);

		ReleaseObj	obj;
		obj.func	=	enCF_Surface_Release;
		obj.pFunc	=	NULL;
		obj.pObj	=	pThis;
...
//		//ULONG uiRef = ST_CubeSurface_Release(pThis) - 1;
//
//		ReleaseObj obj;
//		obj.func	= enCF_CubeSurface_Release;
//		obj.pFunc	= NULL;
//		obj.pObj	= pThis;
//
//		MT_RenderThread::GetSingleton().GetDBuffer().PushReleaseObj(obj);
...
ULONG	STDMETHODCALLTYPE MT_VolumeRelease(IDirect3DVolume9* pThis){
	if(pThis!=NULL){
		pThis->lpVtbl->AddRef(pThis);
		ULONG	uiRef	=	ST_Volume_Release(pThis)-1;
		ReleaseObj	obj;
		obj.func	=	enCF_Volume_Release;
		obj.pFunc	=	NULL;
		obj.pObj	=	pThis;
...
ULONG STDMETHODCALLTYPE	MT_VertexBuffer_Release(IDirect3DVertexBuffer9* pThis)
{
	if (pThis != NULL)
	{
		// 渲染线程处理: 直接调用原函数
		if (g_uiMainThreadId != GetCurrentThreadId())
			return ST_VertexBuffer_Release(pThis);

		// 主线程处理: 加入RenderCommandBuffer 
		//pThis->lpVtbl->AddRef(pThis);
		//ULONG uiRef =	ST_VertexBuffer_Release(pThis) - 1;
		ReleaseObj	obj;
		obj.func	=	enCF_VertexBuffer_Release;
		obj.pFunc	=	NULL;
		obj.pObj	=	pThis;
...
ULONG STDMETHODCALLTYPE MT_IndexBufferRelease(IDirect3DIndexBuffer9* pThis)
{
	if (pThis != NULL)
	{
		// 主线程处理: 加入RenderCommandBuffer 
		//pThis->lpVtbl->AddRef(pThis);
		//ULONG	uiRef	=	ST_IndexBuffer_Release(pThis)-1;
		ReleaseObj	obj;
		obj.func	= enCF_IndexBuffer_Release;
		obj.pFunc	= NULL;
		obj.pObj	= pThis;
...
ULONG STDMETHODCALLTYPE	MT_VertexDeclarationRelease(IDirect3DVertexDeclaration9* pThis)
{
	if (pThis != NULL)
	{
		// 主线程处理: 加入RenderCommandBuffer 
		//pThis->lpVtbl->AddRef(pThis);
		//ULONG	uiRef =	ST_VertexDeclaration_Release(pThis)-1;
		ReleaseObj	obj;
		obj.func	= enCF_VertexDeclaration_Release;
		obj.pFunc	= NULL;
		obj.pObj	= pThis;
...
HRESULT STDMETHODCALLTYPE MT_Texture_LockRect(IDirect3DTexture9* pThis, UINT Level, D3DLOCKED_RECT* pLockedRect, CONST RECT* pRect, DWORD Flags)
{
	...
	U32 oldPitch = uiTempWidth * uiFormatSize / 8; 	
	pLockedRect->Pitch	= oldPitch;	///< 计算行距
...
HRESULT STDMETHODCALLTYPE MT_VolumeTexture_LockBox(IDirect3DVolumeTexture9*	pThis, UINT Level, D3DLOCKED_BOX* pLockedVolume, CONST D3DBOX* pBox, DWORD Flags)
{
	...
	U32	uiBufferSize	= desc.Width*desc.Height*desc.Depth;
	if(pBox!=NULL)
		uiBufferSize	= (pBox->Right - pBox->Left) * (pBox->Bottom - pBox->Top) * (pBox->Back - pBox->Front);
	uiBufferSize		*=uiFormatSize;
	uiBufferSize		/=8;
...
//	param->pID3DXEffect	= pThis;
//	param->Flags		= Flags;
//	*pPasses			= 1;		///< Not Allow Mutli Pass!So Return 1;
//	return S_OK;
//};
//-------------------------------------------------------------------------------------------------
