...
DWORD			g_uiMainThreadId	=	0;
DWORD			g_uiRenderThreadId	=	0xffffffff;
...
"enCF_DeviceRelease",
"enCF_SurfaceRelease",
"enCF_CubeSurfaceRelease",
"enCF_VolumeRelease",
"enCF_VertexBufferRelease",
"enCF_IndexBufferRelease",
"enCF_VertexDeclarationRelease",
...
T_DeviceRelease								ST_DeviceRelease			=	NULL;
T_SurfaceRelease							ST_SurfaceRelease			=	NULL;
T_CubeSurfaceRelease						ST_CubeSurfaceRelease		=	NULL;
T_VolumeRelease								ST_VolumeRelease			=	NULL;
T_VertexBufferRelease						ST_VertexBufferRelease		=	NULL;
T_IndexBufferRelease						ST_IndexBufferRelease		=	NULL;
T_VertexDeclarationRelease					ST_VertexDeclarationRelease	=	NULL;
T_StateBlockRelease							ST_StateBlockRelease		=	NULL;
...
bool	HasHooked(void* p){
	...
	std::set<void*>::iterator	i	=	setHookApi.find(p);
	if(i!=setHookApi.end()){
		return	true;
	}else{
		setHookApi.insert(p);
...
void	Hook_IDirect3DSurface9(IDirect3DDevice9* pDevice){
	...
	ST_SurfaceRelease		=	pSurface->lpVtbl->Release;
	...
	DetourAttach(&(PVOID&)	ST_SurfaceRelease				,MT_SurfaceRelease				);
	DetourTransactionCommit();

	ST_SurfaceRelease(pSurface);
...
void	Hook_IDirect3DCubeSurface9(IDirect3DDevice9* pDevice){
	...
	ST_CubeSurfaceRelease		=	pSurface->lpVtbl->Release;
	...
	//DetourAttach(&(PVOID&)	ST_CubeSurfaceRelease				,MT_CubeSurfaceRelease				);
	DetourTransactionCommit();
...
void	Hook_IDirect3DVolume9(IDirect3DDevice9* pDevice){
	...
	ST_VolumeRelease		=	pVolume->lpVtbl->Release;

	DetourTransactionBegin();
	DetourUpdateThread(GetCurrentThread());
	DetourAttach(&(PVOID&)	ST_Volume_LockBox			,MT_Volume_LockBox			);
	DetourAttach(&(PVOID&)	ST_Volume_UnlockBox			,MT_Volume_UnlockBox			);
	//DetourAttach(&(PVOID&)	ST_VolumeRelease			,MT_VolumeRelease			);
	DetourTransactionCommit();

	ST_SurfaceRelease((IDirect3DSurface9*)pVolume);
	pTex->lpVtbl->Release(pTex);
...
void	Hook_IDirect3DVertexBuffer9(IDirect3DDevice9* pDevice){
	...
	pDevice->lpVtbl->CreateVertexBuffer(pDevice,32,0,D3DUSAGE_DYNAMIC,D3DPOOL_DEFAULT,&pVB,0);
	ST_VertexBuffer_Lock		=	pVB->lpVtbl->Lock;
	ST_VertexBuffer_Unlock		=	pVB->lpVtbl->Unlock;
	ST_VertexBufferRelease		=	pVB->lpVtbl->Release;
	...
	DetourAttach(&(PVOID&)	ST_VertexBufferRelease				,MT_VertexBufferRelease			);
	DetourTransactionCommit();

	ST_VertexBufferRelease(pVB);
...
void	Hook_IDirect3DIndexBuffer9(IDirect3DDevice9* pDevice){
	...
	ST_IndexBufferRelease		=	pIB->lpVtbl->Release;
	...
	//DetourAttach(&(PVOID&)	ST_IndexBufferRelease			,MT_IndexBufferRelease			);
	DetourTransactionCommit();

	ST_VertexBufferRelease((IDirect3DVertexBuffer9*) pIB);
...
void	Hook_IDirect3DDevice9(IDirect3DDevice9*	pThis){
		...
		ST_DeviceRelease						=	pThis->lpVtbl->Release;
		...
		DetourTransactionBegin();
		DetourUpdateThread(GetCurrentThread());
			DetourAttach(&(PVOID&)	ST_DeviceRelease					 ,MT_DeviceRelease					);
...
void	UnHook_IDirect3DDevice9(){
	if(!bHook_IDirect3DDevice9_Finished)
		...
		DetourTransactionBegin();
		DetourUpdateThread(GetCurrentThread());
			DetourDetach(&(PVOID&)	ST_DeviceRelease					 ,MT_DeviceRelease					);
...
MT_RenderThread::~MT_RenderThread(){
	...
	U32	uiSize	=	m_vecVD.size();
	for(U32 i=0;i<uiSize;i++){
		MT_IDirect3DVertexDeclaration9*	p	=	m_vecVD[i];
...
void	MT_RenderThread::ReleaseRenderObject(){
	ReleaseObjVector&	vec	=	m_DBuffer.GetReadReleaseObjVector();
	U32	uiSize	=	vec.size();
	for(U32	i=0;i<uiSize;i++){
		ReleaseObject(vec[i]);
	}
...
void	MT_RenderThread::ReleaseAllObject(){
	...
	vec.clear();
	ReleaseObjVector&	vec1	=	m_DBuffer.GetWriteReleaseObjVector();
	uiSize	=	vec1.size();
	for(U32	i=0;i<uiSize;i++){
		ReleaseObject(vec1[i]);
	}
	vec1.clear();
}
...
void MT_RenderThread::ReleaseObject( ReleaseObj& obj )
{
	...
	case enCF_SurfaceRelease:{
		ST_SurfaceRelease((IDirect3DSurface9*)obj.pObj);
							 }break;
	case enCF_CubeSurfaceRelease:{
		ST_CubeSurfaceRelease((IDirect3DSurface9*)obj.pObj);					 
								 }break;
	case enCF_VolumeRelease:{
		ST_VolumeRelease((IDirect3DVolume9*)obj.pObj);	
							}break;
	case enCF_VertexBufferRelease:{
		ST_VertexBufferRelease((IDirect3DVertexBuffer9*)obj.pObj);	
								  }break;
	case enCF_IndexBufferRelease:{
		ST_IndexBufferRelease((IDirect3DIndexBuffer9*)obj.pObj);	
								 }break;
	case enCF_VertexDeclarationRelease:{
		ST_VertexDeclarationRelease((IDirect3DVertexDeclaration9*)obj.pObj);	
									   }break;
	case enCF_StateBlockRelease:{
		MT_IDirect3DStateBlock9* pStateBlock	=	(MT_IDirect3DStateBlock9*)obj.pObj;
		T_Release(pStateBlock->m_pIDirect3DStateBlock9);
		//ST_StateBlockRelease(pStateBlock->m_pIDirect3DStateBlock9);
		delete pStateBlock;
								}break;
	}
}
...
MT_IDirect3DVertexDeclaration9*	MT_RenderThread::CreateVD(CONST D3DVERTEXELEMENT9*	pVertexElements){
	...
	for(U32 i=0;i<uiSize;i++){
		MT_IDirect3DVertexDeclaration9*	p	=	m_vecVD[i];
		if(p!=NULL){
			U32	uiTempElementCount	=	p->m_vecElement.size();
			if(uiTempElementCount!=uiNumElement)
				continue;
			if(memcmp(pVertexElements,&p->m_vecElement[0],sizeof(D3DVERTEXELEMENT9)*uiNumElement)==0){
				return	p;			
			}
		}
	}
